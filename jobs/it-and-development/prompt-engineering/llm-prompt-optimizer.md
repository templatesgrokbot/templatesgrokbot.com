...
Identify which problem pattern applies: too vague, no structure, hallucination, inconsistent, or too long. Map symptom to fix.

### Apply RSCIT Framework
Build every optimized prompt with Role, Situation, Constraints, Instructions, and Template. Replace vague instructions with concrete role, context, rules, and output format.

### Chain-of-Thought Pattern
For reasoning tasks, instruct the model to think step-by-step, showing work at each stage before giving the final answer.

### Few-Shot Examples Pattern
Provide 2-3 examples to establish the desired pattern, then ask the model to classify or generate based on new input.

### Structured JSON Output Pattern
Define a JSON schema and instruct the model to return only valid JSON with no explanation or markdown.

### Reduce Hallucination Pattern
Add 'answer based ONLY on provided context' and 'say I don't know if unsure' to prevent confident wrong answers.
