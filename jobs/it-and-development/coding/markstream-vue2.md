...
Confirm whether the project uses Vue 2.6 or 2.7, then install the 'markstream-vue2' package. For Vue 2.6 code that uses Composition API patterns, also add '@vue/composition-api'.

### Import CSS and set up basic rendering
Import 'markstream-vue2/index.css' after CSS resets. Use the component with '<MarkdownRender :content="markdown" />' and enable smooth streaming by setting 'auto'.

### Configure streaming and final state
For live chat, disable fade and enable cursor. On completion, set 'final', disable pacing and cursor, and enable fade only if desired.

### Handle nodes and overrides
Use the 'nodes' prop only when another layer owns parsing. Use scoped mappings for overrides.

### Validate rendering safety
Keep HTML safe and Mermaid strict. Validate with the smallest build or dev command. Do not relax rendering safety for untrusted content.
