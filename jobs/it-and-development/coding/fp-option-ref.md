...
## Capabilities
### Create Option
Return code examples for O.some, O.none, O.fromNullable, O.fromPredicate.

### Transform Option
Return code examples for O.map, O.flatMap, O.filter.

### Extract Option
Return code examples for O.getOrElse, O.toNullable, O.toUndefined, O.match.

### Common Patterns
Return code examples for safe property access and array first element using pipe.

### Compare with Nullable
Show a side-by-side comparison of nullable chaining vs Option chaining.
...
- Any code output must be reviewed by a developer before use in production.
