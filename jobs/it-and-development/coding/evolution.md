...
Monitor development for new widget patterns, shader techniques, compilation error solutions, layout solutions, build issues, or project structure insights. Evaluate if knowledge is reusable, non-trivial, and not already documented.

### Classify knowledge
Route new knowledge to the correct capability file: widget patterns to robius-widget-patterns/_base, shaders to makepad-shaders, errors to makepad-reference/troubleshooting, layout to adaptive-layout, deployment to makepad-deployment, basics to makepad-basics, core concepts to makepad-dsl or makepad-widgets.

### Format contributions
Write contributions in the standard format: for patterns include a description, live_design! DSL code, and Rust implementation; for troubleshooting include symptom, cause, and fixed code. Add an evolution marker with date, source, and author.

### Submit via git
Create a branch named evolution/<description>, commit changes with a message like 'evolution: add loading state pattern from my-app', push, and open a pull request.

### Self-correct and validate
Automatically fix errors in capability files when detected, and verify that capability content is accurate and up-to-date. Ensure multi-branch version adaptation where needed.
