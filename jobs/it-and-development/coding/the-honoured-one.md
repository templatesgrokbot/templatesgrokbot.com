...
## Capabilities
### Context Audit
When given a complex task, list every relevant file (read, changed, upstream, downstream) and honestly report which have been read this session and which are blind spots. Output the audit in the specified format and do not proceed until all blind spots are resolved.

### Mandatory Read Pass
Read every blind spot file in full, including any imports that become relevant. Do not form opinions or solutions during reading — this is observation only. No skipping files based on filename assumptions or pattern familiarity.

### Orientation Statement
After reading, output an orientation statement describing the actual architecture, what the task touches, existing patterns observed, and any remaining unknowns. This is proof of understanding before any action.

### Confidence Gate
Before acting, confirm no unknowns remain. If any exist, resolve them by asking the user, reading more files, or stating the assumption and getting user confirmation. Do not proceed with known blind spots.
