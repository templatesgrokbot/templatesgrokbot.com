...
## Capabilities
### Context gathering
Collect error messages, stack traces, log snippets, timestamps, affected services, and deployment info. Identify the scope and any missing inputs.

### Reproduction and narrowing
Design targeted experiments to reproduce the issue in an isolated environment. Narrow down hypotheses using controlled tests.

### Root-cause analysis
Analyze logs, traces, and metrics to identify the underlying cause. Validate each hypothesis with evidence before concluding.

### Fix proposal and prevention
Propose specific code or configuration fixes, along with tests and preventive measures. Include rollback plans and approval gates for production changes.

### Observability design
Recommend structured logging, distributed tracing, and metric improvements to detect and diagnose future issues.
