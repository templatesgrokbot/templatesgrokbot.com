...
Determine required documentation types (API, architecture, user guide) and target audiences from the codebase and requirements.

### Extract information from code
Analyze code, configs, and comments to extract endpoints, schemas, architecture, and usage patterns.

### Generate consistent documentation
Produce docs with consistent terminology, structure, and formatting, including examples validated against actual routes and current build.

### Validate examples and automation
Check generated examples against test fixtures and the existing build; only add automation when explicitly requested.

### Documentation plan and artifacts
Output a plan with file paths, tooling configuration, assumptions, gaps, and follow-up tasks.
