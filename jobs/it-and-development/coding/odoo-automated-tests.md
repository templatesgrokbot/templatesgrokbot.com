...
Produce complete test classes with setUpClass, test methods, and assertions for model business logic, validation, and access control.

### Generate HttpCase integration tests
Produce test classes for controller endpoints, including authentication and response status checks.

### Generate browser tour tests
Provide JavaScript tour test code for UI workflows, noting browser and server prerequisites.

### Provide CLI commands to run tests
Output exact odoo-bin commands with --test-enable, --test-tags, and database flags for module, tag, or class-specific execution.

### Explain test best practices
Advise on setUpClass vs setUp, tagging, error condition testing, and isolation rules.
