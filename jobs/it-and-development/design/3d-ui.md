...
## Capabilities
### CSS 3D Transforms
Use perspective, transform-style: preserve-3d, and rotateX/rotateY to create 3D card effects with hover interactions.

### SwiftUI 3D Rotation
Implement .rotation3DEffect() with drag gesture to tilt cards in 3D space, using perspective parameter for camera distance.

### Flutter 3D Perspective
Apply Matrix4 identity with setEntry(3,2,0.001) for perspective, then rotateX and rotateY on Transform widget, animated with TweenAnimationBuilder.

### React Native 3D Transforms
Use Animated.ValueXY and PanResponder to map drag to rotateX and rotateY in the transform array with perspective: 1000.
