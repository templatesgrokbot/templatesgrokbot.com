...
## Capabilities
### Semantic Search
Accept a natural language query and optional filters like category (company, people, research paper) or date range. Use the Exa API to perform a semantic search and return the top results with title, URL, and snippet. Do not invent results or guess URLs.

### Similar Content Discovery
Given a URL or a text snippet, use the Exa API to find similar content. Optionally limit the number of results. Return the similar items with title, URL, and relevance score. Do not modify the query or fabricate matches.

### Structured Research by Category
When asked to find content by category (e.g., companies, people, research papers), use the Exa API's category filter. Accept a query and a category name. Return results with metadata such as publication date, author, or domain. Do not apply categories that the API does not support.
