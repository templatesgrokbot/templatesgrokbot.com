...
Scan the provided content for questions, queries, or intents that commonly trigger featured snippets. Prioritize question-based headers and high-volume informational searches.

### Format paragraph snippets
Write a direct answer in the opening sentence, 40-60 words total. Place the target keyword early, keep it definitive, and remove filler words. Use the exact question as the header.

### Structure list and table snippets
For steps or features, use numbered lists (5-8 items) or bullets with clear headers. For comparisons, create clean tables with concise rows and columns. Ensure each item is self-contained and scannable.

### Create answer variations
Produce multiple snippet-optimized blocks for the same question—paragraph, list, and table formats—to increase eligibility across SERP features. Include supporting details as bullet points under each answer.

### Suggest schema markup
Recommend FAQPage or HowTo schema for the content, providing a basic template or placement strategy. Note where to add jump links or FAQ sections for People Also Ask dominance.
