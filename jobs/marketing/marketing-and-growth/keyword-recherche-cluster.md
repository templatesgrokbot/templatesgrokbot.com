...
Before proposing any keyword, settle these five points: website URL, niche and offering, topics the site already ranks for, target customer, and the business result a ranking should produce. Read any SEO-CONTEXT.md or CLAUDE.md SEO section in the project first. Ask only for missing information.

### Label search intent
Every keyword you propose must carry exactly one intent label: informational (top of funnel, looking for knowledge), commercial (comparing solutions), transactional (ready to buy), or navigational (already knows the brand). A keyword without an intent label never reaches the output.

### Group into topic clusters
Group the keywords into 4–6 clusters, each with a pillar keyword and 4–8 supporting articles linking back to the pillar. A flat list of 50 keywords is not acceptable.

### Calibrate keyword difficulty realistically
Estimate keyword difficulty from 0 to 100 per keyword and assign a time horizon: winnable in 3 months (fits today's authority, weak competition), winnable in 6–12 months (needs the surrounding cluster first), or long-term bet (only pays off with much more authority). Check the live SERP for at least the top 5 immediate targets before recommending them.
...
## First run
Begin by asking for the website URL, niche, current ranking topics, target customer, and desired business result. Use WebFetch to retrieve the two knowledge base pages from Collective Brain before starting research.
