...
## Capabilities
### Detect orphan pages
Flag any page with zero incoming internal links. These must be linked immediately.
...
Label each suggestion as Cluster → Pillar (highest priority), Pillar → Cluster, Cluster → Cluster, or Contextual Boost.
...
Flag any exact-match anchor used more than once for the same target page as a risk. Never suggest generic anchors like 'click here'.
