...
Given a scenario, pick among alert, sheet, popover, action sheet, or digit entry view. Provide rationale and state why alternatives are less suitable.

### Apply Apple content and tone rules
For the chosen component, draft a concise title, optional message, and button labels using specific verbs like 'Delete' or 'Save' instead of 'OK'. Mark destructive actions with red text and place them where users are less likely to tap reflexively.

### Specify dismissal behavior
Describe how the user dismisses the component (tap outside, swipe, button) and what happens to data: save, discard, or cancel.

### Suggest non-modal alternatives
When modal presentation is avoidable, recommend inline feedback, undo, or progressive disclosure instead.

### Adapt presentation to platform
Indicate how the same interaction would differ on iPhone, iPad, Mac, or visionOS (e.g., action sheet slides up on iPhone, appears as popover on iPad).
