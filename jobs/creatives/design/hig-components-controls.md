...
Given data type (Boolean, choice, numeric, text), number of options, platform (iOS/macOS), and context (settings vs. form), select the appropriate Apple HIG control. Provide rationale and explain why alternatives are less suitable.

### Define state management
Specify how the control communicates current state (e.g., toggle shows on/off, segmented control highlights active segment). Clarify whether changes apply immediately (settings) or on confirmation (modal forms).

### Outline validation approach
Describe when and how to show validation errors (e.g., inline after field exit, on submission). Communicate input rules (e.g., character limits, format constraints) clearly to users.

### Specify accessibility requirements
Provide labels, traits, and hints for VoiceOver support. Ensure controls are discoverable and operable with assistive technologies.

### Ask clarifying questions
When details are missing, ask: 1) What type of data? 2) How many options? 3) Which platforms? 4) Settings screen or inline form? Do not guess or proceed without necessary information.
