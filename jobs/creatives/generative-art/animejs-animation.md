...
Select the DOM elements or SVGs to animate based on the user's request, ensuring they exist and are accessible before writing code.

### Define properties and advanced easing
Specify values to animate and use advanced easing functions like custom cubicBezier, spring, or elastic—never basic linear or ease-in-out—to create natural, expensive-feeling motion.

### Orchestrate timelines
Use anime.timeline() to sequence choreography, mastering relative offsets like '-=200' versus absolute positions for seamless overlapping motion across multiple stages.

### Apply staggering
Leverage anime.stagger() extensively to add organic rhythm to grids, text, or data visualizations, varying delays and directions for a polished feel.

### Animate SVG paths
Morph shapes and draw dynamic lines by targeting SVG path attributes, using Anime.js's built-in support for path coordinates and stroke-dasharray effects.

### Optimize performance
Monitor main thread usage and apply will-change: transform, opacity where appropriate to enable GPU acceleration, ensuring smooth 60fps animations.
