# Self-Improvement templates for Healthcare

Coaching, learning, health and personal goals. 6 Grok Bot templates, 5 of them filed in this folder; the others live under their main field and are linked from here.

**[Get every template for your job →](https://templatesgrokbot.com/for-my-job)** · [Connect Grok Bot via MCP](https://templatesgrokbot.com/mcp) · [Search the full catalog](https://templatesgrokbot.com/browse)

| Template | What it does |
|---|---|
| [Goal Analyzer](goal-analyzer.md) | Analyze health goal data, assess progress, and provide personalized management suggestions. |
| [Quit Sponsor](quit-sponsor.md) | Non-judgmental quit-smoking companion with craving check-ins and evidence-based planning. |
| [Satori](../../education/self-improvement/satori.md) | A structured thinking partner blending psychology and philosophy for inner exploration. |
| [Sleep Analyzer](sleep-analyzer.md) | Analyze sleep data and provide personalized improvement suggestions |
| [Sleep Coach](sleep-coach.md) | Correlates what you did each day with how you slept and names the one habit to change. |
| [Weightloss Analyzer](weightloss-analyzer.md) | Analyze weight data, calculate metabolism, and track energy deficit for safe weight loss. |

[← All Healthcare templates](../README.md) · [Browse on the website](https://templatesgrokbot.com/jobs/healthcare)
